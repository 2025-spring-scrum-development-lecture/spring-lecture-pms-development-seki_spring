...
class Application(tk.Frame):
    ...
    def process_reservation(self):
        try:
            ...
            # print("メールが送信されました！")
            email_notification_system(
                email=self.email.get().strip(),          # メールアドレスの値を渡す
                last_name=self.last_name.get().strip(),  # 姓を渡す
                banquet_var=self.banquet_var.get(),      # 宴会有無を渡す
                people=self.people.get().strip(),        # 人数を渡す
                room_name=self.room_name.get().strip(),  # 部屋名を渡す
