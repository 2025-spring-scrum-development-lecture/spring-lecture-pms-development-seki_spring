...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # 宴会の有無
        self.label_party = tk.Label(self, text="宴会の有無")
        self.label_party.place(x=10, y=90)

        self.party_var = tk.StringVar(value="なし")

        self.radio_button_yes = tk.Radiobutton(self, text="あり", variable=self.party_var, value="あり")
        self.radio_button_yes.place(x=150, y=90)

        self.radio_button_no = tk.Radiobutton(self, text="なし", variable=self.party_var, value="なし")
        ...
        self.num_people = ttk.Combobox(self, values=[str(i) for i in range(1, 11)], width=5)
        self.num_people.place(x=150, y=130)
        ...
        self.checkin_date = DateEntry(self, width=12, mindate=date.today())
        self.checkin_date.place(x=150, y=170)

        # チェックアウトの日
        self.label_checkout = tk.Label(self, text="チェックアウトの日")
        self.label_checkout.place(x=10, y=210)

        self.checkout_date = DateEntry(self, width=12, mindate=date.today())
        self.checkout_date.place(x=150, y=210)
        ...
        self.text_remarks = tk.Text(self, height=5, width=40)
        self.text_remarks.place(x=150, y=390)

        # 見積もり料金
        self.label_estimatedfee_text = tk.Label(self, text="見積もり料金")
        self.label_estimatedfee_text.place(x=10, y=490)

        self.label_estimatedfee = tk.Label(self, text="sample")
        self.label_estimatedfee.place(x=150, y=490)
        ...
        # 内容をリセットボタン
        self.button_reset = tk.Button(self, text="内容をリセット")
